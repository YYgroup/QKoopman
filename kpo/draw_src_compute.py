...
def qkm_true(input,p,net):
    ...
    state = job.result().get_statevector(qc0) # get_statevector  statevector_simulator
    ...
    state = job.result().get_statevector(qc1) # get_statevector  statevector_simulator
    ...
    state = job.result().get_statevector(qc2) # get_statevector  statevector_simulator
    ...
    state = job.result().get_statevector(qc3) # get_statevector  statevector_simulator
    ...
    state = job.result().get_statevector(qc4) # get_statevector  statevector_simulator
...
def getenergy(v):
    ...
    Nx=v.shape[-2] 
    Ny=v.shape[-1]
    kx = np.fft.fftfreq(Nx,d=1/Nx) #  -N/2  N/2
...
def getstructure_vort(v,q_list=[0.5,1.0,1.5,2.0]):
    ...
    Args:
        v (np.array): (t,h,h)
        q_list (list[float]):  2q
    Returns:
        dict: ，：
            {
                'r': (np.array): (h-1,)
                's0.1': (np.array): (t,h-1) q=0.1 0.2
                's0.5': (np.array): (t,h-1) q=0.5 1.0
                ...
                }
            }
    '''       
