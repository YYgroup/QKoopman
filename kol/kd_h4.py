        ...
        if len(paths)==1:
            for path in paths:
                with h5py.File(path,'r') as f:
                    vort_list.append(torch.from_numpy(f['vort'][:168,:,:,:].astype(np.float32)))
                    u0_list.append(torch.from_numpy(f['u0'][:168,:,:,:].astype(np.float32)))
                    v0_list.append(torch.from_numpy(f['v0'][:168,:,:,:].astype(np.float32)))
        else:
            for path in paths:
                 with h5py.File(path,'r') as f:
                    vort_list.append(torch.from_numpy(f['vort'][:,:,:,:].astype(np.float32)))
                    u0_list.append(torch.from_numpy(f['u0'][:,:,:,:].astype(np.float32)))
                    v0_list.append(torch.from_numpy(f['v0'][:,:,:,:].astype(np.float32)))
...
class MixFFN(nn.Module):
    '''AttentionBlock,(B,H*W,in_features),(B,H*W,out_features)
    '''
...
class Attention(nn.Module):
    '''AttentionBlock,->(B,H*W,dim)
     SegFormer: Simple and Efficient Design for Semantic Segmentation with Transformers
    ,Efﬁcient Self-Attention,Rsr_ratio^2
...
class AttentionBlock(nn.Module):
    '''DCTBlock,(B,H*W,in_dim),(B,H*W,dim)
    '''
...
class OverlapEmbedding(nn.Module):
    '''FEBlock,(B,in_chans,img_size,img_size),(B,H*W,embed_dim)
...
class FEBlock(nn.Module):
    ...
    OverlapEmbedding(B,H*W,features),
    DCTBlock(B,H*W,features),
    view(B,features,H,W)
    patch_size -> H = W = (img_size-1)//stride + 1
    '''
...
def train():
    ...
    """
     deepspeed *.py < input.txt > run.log 2>&1
